...
export async function GET(request: Request) {
  ...
  if (fields.response_type !== "code" || fields.client_id !== "https://chatgpt.com" || !isAllowedChatGptRedirect(fields.redirect_uri) || fields.code_challenge_method !== "S256" || !fields.code_challenge) {
...
export async function POST(request: Request) {
  ...
  if (get("response_type") !== "code" || get("client_id") !== "https://chatgpt.com" || !isAllowedChatGptRedirect(redirectUri) || get("code_challenge_method") !== "S256" || !get("code_challenge")) return new NextResponse("Invalid OAuth request",{status:400});
  ...
  const code = signPayload({ typ:"code", iat:now, exp:now+300, client_id:"https://chatgpt.com", redirect_uri:redirectUri, code_challenge:get("code_challenge"), scope:normalizeScopes(get("scope")) });
